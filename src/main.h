...
struct CAddressIndexAmount {
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        ...
        READWRITE(*const_cast<int64_t*>(&value));
    }
};

struct CAddressIndexValue {
    std::vector<CAddressIndexAmount> vin;
    std::vector<CAddressIndexAmount> vout;
    int64_t timestamp;
    ...
    template <typename Stream, typename Operation>
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(*const_cast<std::vector<CAddressIndexAmount>*>(&vin));
        READWRITE(*const_cast<std::vector<CAddressIndexAmount>*>(&vout));
        READWRITE(*const_cast<int64_t*>(&timestamp));
    }
};
